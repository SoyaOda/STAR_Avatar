"""
Blender Script: Generate Clothed 3D Models Only

This script uses Blender to:
1. Load STAR body model
2. Apply random shape & pose
3. Add clothing with physics simulation
4. Export clothed 3D model as OBJ/NPY

Output files are then used with existing Pyrender pipeline for fast 2D rendering.

Usage:
    blender --background --python generate_clothed_3d_models.py -- \
        --star_model ../data/star_models/female/model.npz \
        --clothing_mesh ../clothing/sports_outfit.obj \
        --output_dir ../data/clothed_3d_models \
        --num_samples 10
"""
...
class ClothedModelGenerator:
    """
    Generate clothed 3D STAR models with physics simulation.

    Exports OBJ files for use with Pyrender rendering pipeline.
    """
    ...
    def _setup_scene(self):
        """Setup Blender scene (no rendering needed)."""
    ...
    def _create_skeleton(self):
        """Create simplified skeleton for pose application."""
        # Get joint locations
    ...
    def load_clothing_mesh(self, clothing_path):
    ...
    def apply_cloth_simulation(self, frames=50):
        ...
        Args:
            frames: Number of simulation frames (more = better settling)
        """
        ...
        # Bake cloth simulation for each clothing object
    ...
    def export_clothed_model(self, output_path):
        ...
        # Also export vertices/faces as numpy arrays for direct Pyrender use
        # Apply modifiers and get final mesh
        depsgraph = bpy.context.evaluated_depsgraph_get()

        # Get body mesh with modifiers applied
...
def main():
    """Main pipeline."""
    parser = argparse.ArgumentParser(description='Generate Clothed 3D Models')
    parser.add_argument('--star_model', type=str, required=True)
    parser.add_argument('--clothing_mesh', type=str, nargs='+')
    parser.add_argument('--output_dir', type=str, default='../data/clothed_3d_models')
    parser.add_argument('--num_samples', type=int, default=10)
    parser.add_argument('--cloth_frames', type=int, default=50,
                       help='Cloth simulation frames (more = better quality)')
    ...
    print("\n" + "="*70)
    print("Generate Clothed 3D Models (Blender)")
    print("="*70)
    ...
    for i in range(args.num_samples):
        print(f"\n[{i+1}/{args.num_samples}] Generating clothed model...")

        # Initialize generator
        generator = ClothedModelGenerator(
        ...
        if args.clothing_mesh:
            for clothing_path in args.clothing_mesh:
                generator.load_clothing_mesh(clothing_path)

            # Run cloth simulation
            generator.apply_cloth_simulation(frames=args.cloth_frames)

        # Export clothed model
        output_path = os.path.join(args.output_dir, f"clothed_{i+1:04d}")
        generator.export_clothed_model(output_path)

        # Save parameters
        param_path = os.path.join(args.output_dir, f"clothed_{i+1:04d}_params.pkl")
    ...
    print("\n" + "="*70)
    print("✓ All clothed 3D models generated!")
    print("="*70)
    print(f"\nOutput: {args.output_dir}")
    print("\nNext step: Render with Pyrender")
    print("  python render_clothed_models_with_pyrender.py \\")
    print(f"      --input_dir {args.output_dir} \\")
    print("      --output_dir outputs/pyrender_clothed \\")
    print("      --use_hdri")
    print()
